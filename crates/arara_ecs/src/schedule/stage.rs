...
use arara_utils::{tracing::info, HashMap, HashSet};
...
impl SystemStage {
    ...
    pub fn apply_buffers(&mut self, world: &mut World) {
        for container in self.parallel.iter_mut() {
            ...
            #[cfg(feature = "trace")]
            let span = arara_utils::tracing::info_span!("system_commands", name = &*system.name());
            #[cfg(feature = "trace")]
            ...
            while run_system_loop {
                ...
                for container in &mut self.exclusive_at_start {
                    if should_run(container, &self.run_criteria, default_should_run) {
                        #[cfg(feature = "trace")]
                        let system_span = arara_utils::tracing::info_span!(
                            "exclusive_system",
                            name = &*container.name()
                        );
                        #[cfg(feature = "trace")]
                ...
                if self.apply_buffers {
                    for container in &mut self.parallel {
                        if container.should_run {
                            #[cfg(feature = "trace")]
                            let span = arara_utils::tracing::info_span!(
                                "system_commands",
                                name = &*container.name()
                            );
                            #[cfg(feature = "trace")]
