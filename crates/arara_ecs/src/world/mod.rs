...
impl Default for World {
    fn default() -> Self {
        Self {
            id: WorldId::new().expect("More `arara` `World`s have been created than is supported"),
...
impl World {
    ...
    ///
    /// ```
    /// use arara_ecs::{component::Component, world::World};
    ///
    /// #[derive(Component)]
    /// struct Position {
    ///   x: f32,
    ...
    ///
    /// ```
    /// use arara_ecs::{component::Component, entity::Entity, world::World};
    ///
    /// #[derive(Component)]
    ...
    /// run queries on the [`World`] by storing and reusing the [`QueryState`].
    /// ```
    /// use arara_ecs::{component::Component, entity::Entity, world::World, query::With};
    ///
    /// #[derive(Component)]
    /// struct A;
    /// #[derive(Component)]
    ...
    ///
    /// ```
    /// use arara_ecs::{entity::Entity, world::World, component::Component};
    /// #[derive(Component)]
    /// struct A(&'static str);
    /// #[derive(Component, PartialEq, Debug)]
    /// struct B(f32);
    ...
    /// world access
    /// ```
    /// use arara_ecs::{component::Component, world::{World, Mut}};
    /// #[derive(Component)]
    /// struct A(u32);
    /// #[derive(Component)]
    /// struct B(u32);
...
// TODO: remove allow on lint - https://github.com/araraengine/arara/issues/3666
