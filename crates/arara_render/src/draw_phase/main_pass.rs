...
use crate::{BPLight, ClearColor, Color, Image, Shaders, Visibility, geometry::Mesh};
...
pub fn main_pass(
    ...
    query: Query<(&Handle<Mesh>, &GlobalTransform, &Color, &Option::<Handle<Image>>, &Visibility)>,
) {
    ...
    for (mesh, global_transform, color, image_handle, visibility) in query.iter() {
